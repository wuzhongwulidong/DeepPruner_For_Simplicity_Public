...
class MaxDisparityPredictor(HourGlass):
    ...
    def forward(self, input, input_disparity):
        ...
        Args:
            :input: Left and Warped right Image features as Cost Volume. # [B, 2C+1, n, H, W]
            :input_disparity: PatchMatch predicted disparity samples.    # [B, n, H, W]
        Returns:
        ...
        output2 = self.last_conv3d_layer(output1).squeeze(1)  # [B, 1, n, H, W]->[B, n, H, W]
        feature_output = output2

        confidence_output = self.softmax(output2)  # [B, n, H, W], 在dim=1维度上做SoftMax。
        disparity_output = torch.sum(confidence_output * input_disparity, dim=1).unsqueeze(1)

        return disparity_output, feature_output  # [B, 1, H, W], [B, n, H, W]
...
class MinDisparityPredictor(HourGlass):
    ...
    def forward(self, input, input_disparity):
        ...
        confidence_output = self.softmax(output2)  # [B, n, H, W], 在dim=1维度上做SoftMax。
        # [B, n, H, W] * [B, n, H, W]->sum->[B, H, W]->[B, 1, H, W]
        disparity_output = torch.sum(confidence_output * input_disparity, dim=1).unsqueeze(1)
    ...
    def forward(self, input, input_disparity):
        ...
        Args:
            :input:   Cost-Volume composed of left image features, warped right image features,
                      Confidence range Predictor features and input disparity samples  # [B, 2C+1, n, H, W]

            :input_disparity: input disparity samples. [B, n, H, W]
