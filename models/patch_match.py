...
class DisparityInitialization(nn.Module):
    ...
    def forward(self, min_disparity, max_disparity, number_of_intervals=10):
        ...
        Args:
            ...
            :number_of_intervals (default: 10): Number of samples to be generated. 等于sample_count!
        Returns:
        ...
        multiplier = 1.0 / number_of_intervals
        range_multiplier = torch.arange(0.0, 1, multiplier, device=device).view(number_of_intervals, 1, 1)  # [n,1,1]
        range_multiplier = range_multiplier.repeat(1, min_disparity.size()[2], min_disparity.size()[3])  # [n, H, W]

        interval_noise = min_disparity.new_empty(min_disparity.size()[0], number_of_intervals, min_disparity.size()[2],
                                                 min_disparity.size()[3]).uniform_(0, 1)  # [B, n, H, W]
        interval_min_disparity = min_disparity + (max_disparity - min_disparity) * range_multiplier  # [B, n, H, W]
...
class Evaluate(nn.Module):
    ...
    def forward(self, left_input, right_input, disparity_samples, normalized_disparity_samples):
        ...
        Args:
            :left_input: Left Image Feature Map  [B, C, H, W]
            :right_input: Right Image Feature Map
            :disparity_samples: Disparity Samples to be evaluated. For each pixel, we have
                                ("number of intervals" X "number_of_samples_per_intervals") samples. 尺寸：[B, n*3, H, W]

            :normalized_disparity_samples: 尺寸：[B, n*3, H, W]
        Returns:
        ...
        # [B, C, H, W]->[n*3, B, C, H, W]->[B, C, n*3, H, W]
        ...
        warped_right_feature_map = torch.gather(right_feature_map,  # [B, C, n*3, H, W]
                                                dim=4,
                                                index=right_y_coordinate.expand(  # [B, n*3, H, W]->[C, B, n*3, H, W]->[B, C, n*3, H, W]
                                                    right_input.size()[1], -1, -1, -1, -1).permute([1, 0, 2, 3, 4]).long())

        # [B, C, n*3, H, W] * # [B, C, n*3, H, W] ->mean->[B, n*3, H, W] * 温度 7。 strength实际表示对应像素特征的相似程度。
...
class Propagation(nn.Module):
    ...
    def forward(self, disparity_samples, device, propagation_type="horizontal"):
        ...
        disparity_samples = disparity_samples.view(disparity_samples.size()[0],  # [B, n, H, W] -> [B, 1, n, H, W]。n为number_of_intervals。
                                                   1,
                                                   disparity_samples.size()[1],
                                                   disparity_samples.size()[2],
                                                   disparity_samples.size()[3])
        ...
        return aggregated_disparity_samples  # [B, n*3, H, W]
...
class PatchMatch(nn.Module):
    ...
    def forward(self, left_input, right_input, min_disparity, max_disparity, sample_count=10, iteration_count=3):
        ...
        device = left_input.get_device()
        min_disparity = torch.floor(min_disparity)  # [B, 1, H, W]
        max_disparity = torch.ceil(max_disparity)   # [B, 1, H, W]
        ...
        normalized_disparity_samples, min_disp_tensor, multiplier = self.disparity_initialization(  # [B, n, H, W]
            min_disparity, max_disparity, sample_count)
        # [B, n, H, W] -> [B, n, 1, H, W]->[B, n, 3, H, W] -> [B, n*3, H, W]
        ...
        for prop_iter in range(iteration_count):
            ...
            # [B, n * 3, H, W] * [B, 1, H, W] + [B, n*3, H, W] = [B, n*3, H, W]
        ...
        return disparity_samples  # [B, n, H, W]
